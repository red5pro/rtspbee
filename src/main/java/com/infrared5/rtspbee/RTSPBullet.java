...
import com.red5pro.rtsp.rtp.RTPStreamPacket;
import com.red5pro.rtsp.rtp.handlers.codec.AAC;
import com.red5pro.rtsp.sdp.SDPTrack;
import com.red5pro.rtsp.sdp.SessionDescription;
import com.red5pro.rtsp.sdp.SessionDescriptionProtocolDecoder;
...
public class RTSPBullet implements Runnable {
    ...
    private static final int[] AAC_SAMPLERATES = { 96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350 };
        ...
        try {
            ...
            while (doRun && (k = requestSocket.getInputStream().read()) != -1) {
                ...
                if (k == 36) {
                    ...
                    long time = ((packet[4] & 0xFF) << 24 | (packet[5] & 0xFF) << 16 | (packet[6] & 0xFF) << 8 | (packet[7] & 0xFF)) & 0xFFFFFFFFL;
        ...
        if (packet.payload.length > 3) {
            ...
            if (packetBuffer != null) {
                ...
                if ((packet.payload.length - 4) + packetBufferOffset >= audioDataSize) {
                    ...
                    try {
                        System.arraycopy(packet.payload, 4, packetBuffer, packetBufferOffset, audioDataSize - packetBufferOffset);
                    } catch (Exception e) {
            ...
            } else if (packet.payload.length < audioDataSize + headerBytesLength + 2/* header-bits size */) {
                ...
                // next audio size;
                try {
                    int nxtAudioSize = (packet.payload[(audioDataSize + 4) + 2] & 0xFF) << 8 | (packet.payload[(audioDataSize + 4) + 3] & 0xFF);
    ...
    public String toString() {
        return StringUtils.join(new String[] { "(bullet #" + this.order + ")", "URL: " + this.host, "PORT: " + this.port, "APP: " + this.contextPath,
